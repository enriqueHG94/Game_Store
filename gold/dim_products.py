...
spark = SparkSession.builder.appName("dim_products") \
...
dim_products = spark.read.format("net.snowflake.spark.snowflake") \
    .options(**sfOptionsSilver) \
    .option("dbtable", "CSV.TRF_PRODUCTS") \
    .load()

# Write on the gold base
dim_products.write.format("net.snowflake.spark.snowflake") \
    .options(**sfOptionsGold) \
    .option("dbtable", "csv.dim_products") \
    .mode("append") \
    .save()
